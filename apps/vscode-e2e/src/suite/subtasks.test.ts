...
suite.skip("Mojo Code Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(MojoCodeEventName.Message, ({ taskId, message }) => {
		...
		api.on(MojoCodeEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
