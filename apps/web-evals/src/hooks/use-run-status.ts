...
import { type TokenUsage, MojoCodeEventName, taskEventSchema } from "@Mojo-code/types"
import type { Run, Task, TaskMetrics } from "@Mojo-code/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case MojoCodeEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case MojoCodeEventName.TaskTokenUsageUpdated: {
			...
			case MojoCodeEventName.EvalPass:
			case MojoCodeEventName.EvalFail:
