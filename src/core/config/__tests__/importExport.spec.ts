...
import type { ProviderName } from "@Mojo-code/types"
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/Mojo-code-settings.json",
			})
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/Mojo-code-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Documents", "Mojo-code-settings.json"))
		})
