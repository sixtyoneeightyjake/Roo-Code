...
// If a user modifies a file outside of Mojo, the context may become stale and need to be updated.
// We do not want Mojo to reload the context every time a file is modified, so we use this class merely
// to inform Mojo that the change has occurred, and tell Mojo to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Mojo was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to Mojo via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of Mojo, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	private recentlyEditedByMojo = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.onDidChange(() => {
			if (this.recentlyEditedByMojo.has(filePath)) {
				this.recentlyEditedByMojo.delete(filePath) // This was an edit by Mojo, no need to inform Mojo
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Mojo
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to Mojo via a tool, mention, or edit.
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				Mojo_read_date: getLatestDateForField(filePath, "Mojo_read_date"),
				Mojo_edit_date: getLatestDateForField(filePath, "Mojo_edit_date"),
			...
			switch (source) {
				...
				// Mojo_edited: Mojo has edited the file
				case "Mojo_edited":
					newEntry.Mojo_read_date = now
					newEntry.Mojo_edit_date = now
					this.checkpointPossibleFiles.add(filePath)
					this.markFileAsEditedByMojo(filePath)
					break

				// read_tool/file_mentioned: Mojo has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.Mojo_read_date = now
					break
	...
	// Marks a file as edited by Mojo to prevent false positives in file watchers
	markFileAsEditedByMojo(filePath: string): void {
		this.recentlyEditedByMojo.add(filePath)
	}
