// npx vitest core/ignore/__tests__/MojoIgnoreController.spec.ts

import type { Mock } from "vitest"

import { MojoIgnoreController, LOCK_TEXT_SYMBOL } from "../RooIgnoreController"
...
describe("MojoIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: MojoIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new MojoIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .Mojoignore exists
		 */
		it("should load .Mojoignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .Mojoignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".Mojoignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".Mojoignore"), "utf8")

			// Verify content was stored
			expect(controller.MojoIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .Mojoignore doesn't exist
		 */
		it("should allow all access when .Mojoignore doesn't exist", async () => {
			// Setup mocks to simulate missing .Mojoignore file
			...
			expect(controller.MojoIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .Mojoignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .Mojoignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .Mojoignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .Mojoignore exists
		 */
		it("should allow all access when no .Mojoignore content", async () => {
			// Create a new controller with no .Mojoignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new MojoIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .Mojoignore exists
		 */
		it("should allow all commands when no .Mojoignore exists", async () => {
			// Create a new controller with no .Mojoignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new MojoIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .Mojoignore
		 */
		it("should generate formatted instructions when .Mojoignore exists", async () => {
			// Setup .Mojoignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .Mojoignore exists
		 */
		it("should return undefined when no .Mojoignore exists", async () => {
			// Setup no .Mojoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .Mojoignore is created
		 */
		it("should reload .Mojoignore when file is created", async () => {
			// Setup initial state without .Mojoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .Mojoignore
			controller = new MojoIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .Mojoignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .Mojoignore is changed
		 */
		it("should reload .Mojoignore when file is changed", async () => {
			// Setup initial state with .Mojoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .Mojoignore is deleted
		 */
		it("should reset when .Mojoignore is deleted", async () => {
			// Setup initial state with .Mojoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
