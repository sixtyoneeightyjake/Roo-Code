...
export async function parseMentions(
	...
	showMojoIgnoredFiles: boolean = true,
...
async function getFileOrFolderContent(
	...
	try {
		...
		if (stats.isFile()) {
			if (MojoIgnoreController && !MojoIgnoreController.validateAccess(absPath)) {
				return `(File ${mentionPath} is ignored by .Mojoignore)`
			}
			try {
			...
			for (let index = 0; index < entries.length; index++) {
				...
				if (MojoIgnoreController) {
					isIgnored = !MojoIgnoreController.validateAccess(entryPath)
				}

				if (isIgnored && !showMojoIgnoredFiles) {
					continue
				}
