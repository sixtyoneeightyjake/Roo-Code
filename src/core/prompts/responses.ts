...
export const formatResponse = {
	...
	MojoIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .Mojoignore file settings. You must try to continue in the task without using this file, or ask the user to update the .Mojoignore file.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		files: string[],
		didHitLimit: boolean,
		MojoIgnoreController: MojoIgnoreController | undefined,
		showMojoIgnoredFiles: boolean,
		...
		let MojoIgnoreParsed: string[] = sorted

		if (MojoIgnoreController) {
			MojoIgnoreParsed = []
			for (const filePath of sorted) {
				...
				const isIgnored = !MojoIgnoreController.validateAccess(absoluteFilePath)

				if (isIgnored) {
					// If file is ignored and we're not showing ignored files, skip it
					if (!showMojoIgnoredFiles) {
						continue
					}
					// Otherwise, mark it with a lock symbol
					MojoIgnoreParsed.push(LOCK_TEXT_SYMBOL + " " + filePath)
				} else {
					// Check if file is write-protected (only for non-ignored files)
					const isWriteProtected = MojoProtectedController?.isWriteProtected(absoluteFilePath) || false
					if (isWriteProtected) {
						MojoIgnoreParsed.push("🛡️ " + filePath)
					} else {
						MojoIgnoreParsed.push(filePath)
					}
				}
			}
		}
		if (didHitLimit) {
			return `${MojoIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		} else if (MojoIgnoreParsed.length === 0 || (MojoIgnoreParsed.length === 1 && MojoIgnoreParsed[0] === "")) {
