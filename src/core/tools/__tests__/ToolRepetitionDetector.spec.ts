...
import type { ToolName } from "@Mojo-code/types"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key, options) => {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `Mojo appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy.`
		}
		return key
	}),
}))
