...
describe("writeToFileTool", () => {
	...
	describe("access control", () => {
		it("validates and allows access when MojoIgnoreController permits", async () => {
			await executeWriteFileTool({}, { accessAllowed: true })

			expect(mockCline.MojoIgnoreController.validateAccess).toHaveBeenCalledWith(testFilePath)
	...
	describe("file operations", () => {
		it("successfully creates new files with full workflow", async () => {
			...
			expect(mockCline.fileContextTracker.trackFileContext).toHaveBeenCalledWith(testFilePath, "Mojo_edited")
