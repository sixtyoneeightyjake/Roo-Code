...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (MojoIgnoreController && !MojoIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, MojoIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = MojoIgnoreController ? MojoIgnoreController.filterPaths(filesToParse) : filesToParse
	...
	for (const file of markdownFiles) {
		// Check if we have permission to access this file
		if (MojoIgnoreController && !MojoIgnoreController.validateAccess(file)) {
			continue
		}
	...
	for (const file of otherFiles) {
		const definitions = await parseFile(file, languageParsers, MojoIgnoreController)
		if (definitions) {
			result += `# ${path.relative(dirPath, file).toPosix()}\n${definitions}\n`
		}
	}
 ...
 * @param MojoIgnoreController - Optional controller to check file access permissions
